...
public class MyFTP_Server_Worker extends Thread {
    ...
    private String validUser = "vanhuy";
    private String validPassword = "dut123";
...
//    }
        ...
        if (dataConnection == null || dataConnection.isClosed()){
            ...
            if (dirContent == null){
                ...
                for (int i = 0; i < dirContent.length; i++){
                	debugOutput("dirContent[i]: "+dirContent[i]);
    ...
    private String[] nlstHelper(String args){
    	...
    	debugOutput("nlstHelper: ");
        String filename = currDirectory;
        debugOutput("filename1: "+filename);
        if (args != null){
            filename = filename + fileSeparator + args;
            debugOutput("filename2: "+filename);
        }
        ...
        if (f.exists() && f.isDirectory()){
        	debugOutput("f.exists() && f.isDirectory()");
            return f.list();
        }
        else if (f.exists() && f.isFile()){
        	debugOutput("f.exists() && f.isFile()");
            String[] allFiles = new String[1];
            debugOutput("allFiles[0]: "+allFiles[0]);
...
//    }
    ...
    public void handleList1(String args) {
    	if (dataConnection == null || dataConnection.isClosed()){
            sendMsgToClient("425 No data connection was established");
        }
        else{
